...
export default function DayModal({ date, schedule, members, canEdit = true, onClose, onSaved }: Props) {
          ...
          <div style={{ overflowY:'auto', flex:1, padding:'6px 20px 36px', colorScheme:'light' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                {!canEdit && (
                  <span style={{ fontSize:11, fontWeight:700, color:'#9CA3AF', background:'#F3F4F6', padding:'4px 10px', borderRadius:99 }}>
                    🔒 읽기 전용
                  </span>
                )}
                {canEdit && schedule && (
            ...
            {canEdit ? (
              ...
              <div style={{ padding:'10px 14px', borderRadius:12, marginBottom:14, background:'#FFFBEB', border:'1.5px solid #FCD34D', fontSize:14, fontWeight:700, color:'#374151' }}>
                😴 휴무일
              </div>
            ) : null}
            ...
            {canEdit ? (
              ...
              (m1 || m2) && (
                <div style={{ marginBottom:14 }}>
                  <div style={{ fontSize:11, fontWeight:800, color:'#9CA3AF', textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:8 }}>배정된 당번</div>
                  <div style={{ display:'flex', gap:8 }}>
                    {[m1,m2].filter(Boolean).map((id,i) => {
                      const mem = members.find(m=>m.id===id)
                      if(!mem) return null
                      const c = getMemberColor(members.findIndex(m=>m.id===id))
                      return <div key={i} style={{ padding:'10px 18px', borderRadius:99, background:c.bg, color:'#fff', fontSize:14, fontWeight:700 }}>{mem.name}</div>
                    })}
                  </div>
                </div>
              )
            ...
            <div style={{ marginBottom:20 }}>
              ...
              <div style={{ background:'#F9FAFB', borderRadius:12, border:'1.5px solid #E5E7EB', overflow:'hidden' }}>
                  ...
                  <div key={idx} style={{
                    ...
                    {canEdit ? (
                      <button
                        ...
                        style={{ width:38, height:44, flexShrink:0, background: item.emoji ? 'rgba(99,102,241,0.06)' : 'none', border:'none', cursor:'pointer', fontSize:20, display:'flex', alignItems:'center', justifyContent:'center', borderRight:'1px solid #EFEFEF', position:'relative' }}
                      >
                        {item.emoji || <span style={{ fontSize:16, opacity:0.3 }}>😊</span>}
                        {item.emoji && <span style={{ position:'absolute', top:4, right:4, width:5, height:5, borderRadius:'50%', background:'#6366f1', opacity:0.6 }}/>}
                      </button>
                    ) : (
                      <div style={{ width:38, height:44, flexShrink:0, fontSize:20, display:'flex', alignItems:'center', justifyContent:'center', borderRight:'1px solid #EFEFEF' }}>
                        {item.emoji || ''}
                      </div>
                    )}
                    ...
                    {canEdit ? (
                      ...
                      <div style={{ flex:1, padding:'10px 6px', fontSize:14, color:'#1F2937' }}>{item.text}</div>
                    )}

                    {/* 항목 삭제 */}
                    {canEdit && items.length > 1 && (
                      <button onClick={() => setItems(items.filter((_,i) => i !== idx))}
                        style={{ width:30, flexShrink:0, background:'none', border:'none', cursor:'pointer', fontSize:18, color:'#D1D5DB', padding:0 }}>×</button>
                    )}
                  </div>
                ))}
                ...
                {canEdit && (
                  <button
                    onClick={() => { const next = [...items, {emoji:'',text:''}]; setItems(next); setTimeout(()=>inputRefs.current[next.length-1]?.focus(),30) }}
                    style={{ width:'100%', padding:'9px', background:'none', border:'none', borderTop:'1px dashed #E5E7EB', cursor:'pointer', fontSize:12, color:'#9CA3AF', fontWeight:600, display:'flex', alignItems:'center', justifyContent:'center', gap:4 }}
                  >+ 항목 추가</button>
                )}
              </div>
            </div>
            ...
            {canEdit ? (
              <div style={{ display:'flex', gap:10 }}>
                ...
                <button onClick={save} disabled={saving} style={{ flex:1, padding:'13px 0', borderRadius:14, background:saving?'#9CA3AF':'#1a1a1a', color:'#fff', fontSize:15, fontWeight:800, border:'none', cursor:'pointer', boxShadow:saving?'none':'0 4px 14px rgba(0,0,0,0.2)' }}>
                  {saving ? '저장 중…' : '저장하기'}
                </button>
              </div>
            ) : (
              <button onClick={onClose} style={{ width:'100%', padding:'13px', borderRadius:14, background:'#F3F4F6', color:'#6B7280', fontSize:14, fontWeight:700, border:'none', cursor:'pointer' }}>닫기</button>
            )}
          </div>
        </div>
      </div>
